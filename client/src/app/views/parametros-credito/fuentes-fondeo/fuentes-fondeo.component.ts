...
import {FuentesFondeoFormComponent} from "./fuentes-fondeo-form/fuentes-fondeo-form.component";
import {FuentesFondeoDeleteComponent} from "./fuentes-fondeo-delete/fuentes-fondeo-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-fuentes-fondeo',
  templateUrl: './fuentes-fondeo.component.html',
  styleUrls: ['./fuentes-fondeo.component.sass']
})
export class FuentesFondeoComponent implements OnInit {
  public _datos = { _title: 'Fuentes Fondeo', _modulo: 'ParametrosCredito', _icono: 'fas fa-folder-open', _dominio: 'FuentesFondeo', _componente: 'Fuentes Fondeo'}
  displayedColumns = [ 'select',
    'claveInstitucionFondeadora',
    'nombreInstitucionFondeadora',
    'actions' ];

  selection = new SelectionModel<_fuentesFondeo>(true, []);
  advanceTable: _fuentesFondeo | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: FuentesFondeoDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_fuentesFondeo>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(FuentesFondeoFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_fuentesFondeo>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(FuentesFondeoFormComponent, {
    ...
    deleteItem(row) {
      this.id = row.id;
      const dialogRef = this.dialog.open(FuentesFondeoDeleteComponent, { data: row });
      dialogRef.afterClosed().subscribe((result) => {
        if (result) {
  ...
  private refreshTable() { this.paginator._changePageSize(this.paginator.pageSize) }
    ...
    removeSelectedRows() {
      ...
      this.selection.selected.forEach((item) => {
        ...
        this.selection = new SelectionModel<_fuentesFondeo>(true, []);
      });
      this.loadData();
  ...
  public loadData() {
      ...
      this.dataSource = new FuentesFondeoDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
    ...
    onContextMenu(event: MouseEvent, item: _fuentesFondeo) {
  ...
  export class FuentesFondeoDataSource extends DataSource<_fuentesFondeo> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _fuentesFondeo[] = [];
  renderedData: _fuentesFondeo[] = [];
  ...
  constructor(public _dataSource: AdvanceRestService, public _paginator: MatPaginator, public _sort: MatSort, private _dominio: string ) {
    ...
    connect(): Observable<_fuentesFondeo[]> {
      ...
      return merge(...displayDataChanges).pipe( map(() => {
          this.filteredData = this._dataSource.data.slice().filter((advanceTable: _fuentesFondeo) => {
            const searchStr = ( advanceTable.id + advanceTable.claveInstitucionFondeadora + advanceTable.nombreInstitucionFondeadora ).toLowerCase();
  ...
  sortData(data: _fuentesFondeo[]): _fuentesFondeo[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveInstitucionFondeadora':
          [propertyA, propertyB] = [a.claveInstitucionFondeadora, b.claveInstitucionFondeadora];
          break;
        case 'nombreInstitucionFondeadora':
          [propertyA, propertyB] = [a.nombreInstitucionFondeadora, b.nombreInstitucionFondeadora];
          break;
      }
