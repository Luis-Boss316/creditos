...
import {TasasReferenciasFormComponent} from "./tasas-referencias-form/tasas-referencias-form.component";
import {TasasReferenciasDeleteComponent} from "./tasas-referencias-delete/tasas-referencias-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tasas-referencias',
  templateUrl: './tasas-referencias.component.html',
  styleUrls: ['./tasas-referencias.component.sass']
})
export class TasasReferenciasComponent implements OnInit {
  public _datos = { _title: 'Tasas Referencias', _modulo: 'Parametros Credito', _icono: 'fas fa-folder-open', _dominio: 'TasasReferencia', _componente: 'Tasas Referencias'}
  ...
  selection = new SelectionModel<_tasasReferencia>(true, []);
  advanceTable: _tasasReferencia | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TasasReferenciasDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tasasReferencia>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TasasReferenciasFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tasasReferencia>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TasasReferenciasFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TasasReferenciasDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tasasReferencia>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TasasReferenciasDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tasasReferencia) {
...
export class TasasReferenciasDataSource extends DataSource<_tasasReferencia> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tasasReferencia[] = [];
  renderedData: _tasasReferencia[] = [];
  ...
  connect(): Observable<_tasasReferencia[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tasasReferencia) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTasa + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _tasasReferencia[]): _tasasReferencia[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTasa':
          [propertyA, propertyB] = [a.claveTasa, b.claveTasa];
          break;
