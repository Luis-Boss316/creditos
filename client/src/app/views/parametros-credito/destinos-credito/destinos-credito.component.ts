...
import {DestinosCreditoFormComponent} from "./destinos-credito-form/destinos-credito-form.component";
import {DestinosCreditoDeleteComponent} from "./destinos-credito-delete/destinos-credito-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-destinos-credito',
  templateUrl: './destinos-credito.component.html',
  styleUrls: ['./destinos-credito.component.sass']
})
export class DestinosCreditoComponent implements OnInit {
  public _datos = { _title: 'Destinos Credito', _modulo: 'ParametrosCredito', _icono: 'fas fa-folder-open', _dominio: 'DestinosCredito', _componente: 'Destinos Credito'}
  displayedColumns = [ 'select',
    'claveDestino',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_destinosCredito>(true, []);
  advanceTable: _destinosCredito | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: DestinosCreditoDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_destinosCredito>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(DestinosCreditoFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_destinosCredito>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(DestinosCreditoFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(DestinosCreditoDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_destinosCredito>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new DestinosCreditoDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _destinosCredito) {
...
export class DestinosCreditoDataSource extends DataSource<_destinosCredito> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _destinosCredito[] = [];
  renderedData: _destinosCredito[] = [];
  ...
  connect(): Observable<_destinosCredito[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _destinosCredito) => {
          const searchStr = ( advanceTable.id + advanceTable.claveDestino + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _destinosCredito[]): _destinosCredito[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveDestino':
          [propertyA, propertyB] = [a.claveDestino, b.claveDestino];
          break;
        case 'descripcion':
