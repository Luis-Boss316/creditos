...
import {TiposCreditoFormComponent} from "./tipos-credito-form/tipos-credito-form.component";
import {TiposCreditoDeleteComponent} from "./tipos-credito-delete/tipos-credito-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipos-credito',
  templateUrl: './tipos-credito.component.html',
  styleUrls: ['./tipos-credito.component.sass']
})
export class TiposCreditoComponent implements OnInit {
  public _datos = { _title: 'Tipos Credito', _modulo: 'Parametros Credito', _icono: 'fas fa-folder-open', _dominio: 'TiposCredito', _componente: 'Tipos Credito'}
  ...
  selection = new SelectionModel<_tiposCredito>(true, []);
  advanceTable: _tiposCredito | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TiposCreditoDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tiposCredito>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposCreditoFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tiposCredito>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposCreditoFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TiposCreditoDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tiposCredito>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TiposCreditoDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tiposCredito) {
...
export class TiposCreditoDataSource extends DataSource<_tiposCredito> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tiposCredito[] = [];
  renderedData: _tiposCredito[] = [];
  ...
  connect(): Observable<_tiposCredito[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tiposCredito) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoCredito + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _tiposCredito[]): _tiposCredito[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoCredito':
          [propertyA, propertyB] = [a.claveTipoCredito, b.claveTipoCredito];
          break;
