...
import {PeritosFormComponent} from "./peritos-form/peritos-form.component";
import {PeritosDeleteComponent} from "./peritos-delete/peritos-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-peritos',
  templateUrl: './peritos.component.html',
  styleUrls: ['./peritos.component.sass']
})
export class PeritosComponent implements OnInit {
  public _datos = { _title: 'Peritos', _modulo: 'ParametrosCredito', _icono: 'fas fa-folder-open', _dominio: 'Peritos', _componente: 'Peritos'}
  displayedColumns = [ 'select',
    'numeroPerito',
    'numeroRegistroPerito',
    'apellidoPaternoPeritoTitular',
    'apellidoMaternoPeritoTitular',
    'primerNombrePeritoTitular',
    'segundoNombrePeritoTitular',
    'actions' ];

  selection = new SelectionModel<_peritos>(true, []);
  advanceTable: _peritos | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: PeritosDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_peritos>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(PeritosFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_peritos>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(PeritosFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(PeritosDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_peritos>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new PeritosDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _peritos) {
...
export class PeritosDataSource extends DataSource<_peritos> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _peritos[] = [];
  renderedData: _peritos[] = [];
  ...
  connect(): Observable<_peritos[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _peritos) => {
          const searchStr = ( advanceTable.id + advanceTable.numeroPerito + advanceTable.apellidoPaternoPeritoTitular + advanceTable.primerNombrePeritoTitular ).toLowerCase();
  ...
  sortData(data: _peritos[]): _peritos[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'numeroPerito':
          [propertyA, propertyB] = [a.numeroPerito, b.numeroPerito];
          break;
        case 'apellidoPaternoPeritoTitular':
          [propertyA, propertyB] = [a.apellidoPaternoPeritoTitular, b.apellidoPaternoPeritoTitular];
          break;
        case 'primerNombrePeritoTitular':
          [propertyA, propertyB] = [a.primerNombrePeritoTitular, b.primerNombrePeritoTitular];
          break;
      }
