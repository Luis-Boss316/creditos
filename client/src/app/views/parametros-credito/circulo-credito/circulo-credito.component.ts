...
import {CirculoCreditoFormComponent} from "./circulo-credito-form/circulo-credito-form.component";
import {CirculoCreditoDeleteComponent} from "./circulo-credito-delete/circulo-credito-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-circulo-credito',
  templateUrl: './circulo-credito.component.html',
  styleUrls: ['./circulo-credito.component.sass']
})
export class CirculoCreditoComponent implements OnInit {
  public _datos = { _title: 'Circulo Credito', _modulo: 'ParametrosCredito', _icono: 'fas fa-folder-open', _dominio: 'CirculoCredito', _componente: 'Circulo Credito'}
  ...
  selection = new SelectionModel<_circuloCredito>(true, []);
  advanceTable: _circuloCredito | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: CirculoCreditoDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_circuloCredito>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(CirculoCreditoFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_circuloCredito>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(CirculoCreditoFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(CirculoCreditoDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_circuloCredito>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new CirculoCreditoDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _circuloCredito) {
...
export class CirculoCreditoDataSource extends DataSource<_circuloCredito> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _circuloCredito[] = [];
  renderedData: _circuloCredito[] = [];
  ...
  connect(): Observable<_circuloCredito[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _circuloCredito) => {
          const searchStr = ( advanceTable.id + advanceTable.numeroInstitucion + advanceTable.nombreOtorgante + advanceTable.domicilioDevolucion ).toLowerCase();
  ...
  sortData(data: _circuloCredito[]): _circuloCredito[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'numeroInstitucion':
          [propertyA, propertyB] = [a.numeroInstitucion, b.numeroInstitucion];
          break;
        case 'nombreOtorgante':
          [propertyA, propertyB] = [a.nombreOtorgante, b.nombreOtorgante];
          break;
        case 'domicilioDevolucion':
          [propertyA, propertyB] = [a.domicilioDevolucion, b.domicilioDevolucion];
          break;
      }
