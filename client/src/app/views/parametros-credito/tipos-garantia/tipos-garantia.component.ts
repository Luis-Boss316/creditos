...
import {TiposGarantiaFormComponent} from "./tipos-garantia-form/tipos-garantia-form.component";
import {TiposGarantiaDeleteComponent} from "./tipos-garantia-delete/tipos-garantia-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipos-garantia',
  templateUrl: './tipos-garantia.component.html',
  styleUrls: ['./tipos-garantia.component.sass']
})
export class TiposGarantiaComponent implements OnInit {
  public _datos = { _title: 'Tipos Garantia', _modulo: 'Parametros Credito', _icono: 'fas fa-folder-open', _dominio: 'TiposGarantia', _componente: 'Tipos Garantia'}
  ...
  selection = new SelectionModel<_tiposGarantia>(true, []);
  advanceTable: _tiposGarantia | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TiposGarantiaDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tiposGarantia>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposGarantiaFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tiposGarantia>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposGarantiaFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TiposGarantiaDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tiposGarantia>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TiposGarantiaDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tiposGarantia) {
...
export class TiposGarantiaDataSource extends DataSource<_tiposGarantia> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tiposGarantia[] = [];
  renderedData: _tiposGarantia[] = [];
  ...
  connect(): Observable<_tiposGarantia[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tiposGarantia) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoGarantia + advanceTable.descripcion + advanceTable.deudor ).toLowerCase();
  ...
  sortData(data: _tiposGarantia[]): _tiposGarantia[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoGarantia':
          [propertyA, propertyB] = [a.claveTipoGarantia, b.claveTipoGarantia];
          break;
        case 'descripcion':
          [propertyA, propertyB] = [a.descripcion, b.descripcion];
          break;
        case 'deudor':
          [propertyA, propertyB] = [a.deudor, b.deudor];
          break;
      }
