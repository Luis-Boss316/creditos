...
import {BuroCreditoFormComponent} from "./buro-credito-form/buro-credito-form.component";
import {BuroCreditoDeleteComponent} from "./buro-credito-delete/buro-credito-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-buro-credito',
  templateUrl: './buro-credito.component.html',
  styleUrls: ['./buro-credito.component.sass']
})
export class BuroCreditoComponent implements OnInit {
  public _datos = { _title: 'Buro Credito', _modulo: 'ParametrosCredito', _icono: 'fas fa-folder-open', _dominio: 'BuroCredito', _componente: 'Buro Credito'};
  displayedColumns = [ 'select',
    ...
    'claveVersion',
    'domicilioDevolucion',
    'fechaUltimoReporteSemanal',
    'actions' ];

  selection = new SelectionModel<_buroCredito>(true, []);
  advanceTable: _buroCredito | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: BuroCreditoDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_buroCredito>().subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(BuroCreditoFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_buroCredito>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(BuroCreditoFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(BuroCreditoDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_buroCredito>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new BuroCreditoDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _buroCredito) {
...
export class BuroCreditoDataSource extends DataSource<_buroCredito> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _buroCredito[] = [];
  renderedData: _buroCredito[] = [];
  ...
  connect(): Observable<_buroCredito[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _buroCredito) => {
          const searchStr = ( advanceTable.id + advanceTable.nombreOtorgante + advanceTable.domicilioDevolucion ).toLowerCase();
  ...
  sortData(data: _buroCredito[]): _buroCredito[] {
