...
import {ActEcoCnbvFormComponent} from "./act-eco-cnbv-form/act-eco-cnbv-form.component";
import {ActEcoCnbvDeleteComponent} from "./act-eco-cnbv-delete/act-eco-cnbv-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-act-eco-cnbv',
  templateUrl: './act-eco-cnbv.component.html',
  styleUrls: ['./act-eco-cnbv.component.sass']
})
export class ActEcoCNBVComponent implements OnInit {
  public _datos = { _title: 'Actividades Economicas CNBV', _modulo: 'CatalogosOficiales', _icono: 'fas fa-folder-open', _dominio: 'ActEcoCNBV', _componente: 'Actividades Economicas CNBV'};
  displayedColumns = [ 'select', 'claveActividad', 'descripcion', 'actions' ];

  selection = new SelectionModel<_actEcoCNBV>(true, []);
  advanceTable: _actEcoCNBV | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: ActEcoCNBVDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_actEcoCNBV>().subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(ActEcoCnbvFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_actEcoCNBV>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(ActEcoCnbvFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(ActEcoCnbvDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_actEcoCNBV>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new ActEcoCNBVDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _actEcoCNBV) {
...
export class ActEcoCNBVDataSource extends DataSource<_actEcoCNBV> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _actEcoCNBV[] = [];
  renderedData: _actEcoCNBV[] = [];
  ...
  connect(): Observable<_actEcoCNBV[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _actEcoCNBV) => {
  ...
  sortData(data: _actEcoCNBV[]): _actEcoCNBV[] {
