...
import {_actEcoB, _monedas} from "../../../shared/interfaces/Creditos.interface";
...
import {ActEconomicasBanxicoFormComponent} from "./act-economicas-banxico-form/act-economicas-banxico-form.component";
import {ActEconomicasBanxicoDeleteComponent} from "./act-economicas-banxico-delete/act-economicas-banxico-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-act-economicas-banxico',
  templateUrl: './act-economicas-banxico.component.html',
  styleUrls: ['./act-economicas-banxico.component.sass']
})
export class ActEconomicasBanxicoComponent implements OnInit {
  public _datos = { _title: 'Actividades Economicas BANXICO', _modulo: 'CatalogosOficiales', _icono: 'fas fa-folder-open', _dominio: 'ActEcoB', _componente: 'Actividades Economicas BANXICO'}
  displayedColumns = [ 'select', 'numeroSector', 'indicadorActRiesgo', 'actions' ];

  selection = new SelectionModel<_actEcoB>(true, []);
  advanceTable: _actEcoB | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: ActEconomicasBanxicoDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_actEcoB>().subscribe(result => {
      data = result;
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_actEcoB>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(ActEconomicasBanxicoFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(ActEconomicasBanxicoDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_actEcoB>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new ActEconomicasBanxicoDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _actEcoB) {
...
export class ActEconomicasBanxicoDataSource extends DataSource<_actEcoB> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _actEcoB[] = [];
  renderedData: _actEcoB[] = [];
  ...
  connect(): Observable<_actEcoB[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _actEcoB) => {
          const searchStr = ( advanceTable.id ).toString().toLowerCase();
  ...
  sortData(data: _actEcoB[]): _actEcoB[] {
