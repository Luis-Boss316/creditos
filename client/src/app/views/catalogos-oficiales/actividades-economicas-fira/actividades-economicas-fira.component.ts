...
import {ActividadesEconomicasFIRAFormComponent} from "./actividades-economicas-firaform/actividades-economicas-firaform.component";
...
import {ActividadesEconomicasFIRADeleteComponent} from "./actividades-economicas-firadelete/actividades-economicas-firadelete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-actividades-economicas-fira',
  templateUrl: './actividades-economicas-fira.component.html',
  styleUrls: ['./actividades-economicas-fira.component.sass']
})
export class ActividadesEconomicasFIRAComponent implements OnInit {
  public _datos = { _title: 'Actividades Economicas FIRA', _modulo: 'CatalogosOficiales', _icono: 'fas fa-folder-open', _dominio: 'ActEcoF', _componente: 'Actividades Economicas FIRA'}
  displayedColumns = [ 'select',
    'claveActividad',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_actEcoF>(true, []);
  advanceTable: _actEcoF | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: ActEconomicasFIRADataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_actEcoF>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(ActividadesEconomicasFIRAFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_actEcoF>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(ActEconomicasBanxicoFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(ActividadesEconomicasFIRADeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_actEcoF>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new ActEconomicasFIRADataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _actEcoF) {
...
export class ActEconomicasFIRADataSource extends DataSource<_actEcoF> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _actEcoF[] = [];
  renderedData: _actEcoF[] = [];
  ...
  connect(): Observable<_actEcoF[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _actEcoF) => {
          const searchStr = ( advanceTable.id + advanceTable.claveActividad + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _actEcoF[]): _actEcoF[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveActividad':
          [propertyA, propertyB] = [a.claveActividad, b.claveActividad];
          break;
        case 'descripcion':
