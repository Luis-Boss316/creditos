...
import {SectoresBanxicoFormComponent} from "./sectores-banxico-form/sectores-banxico-form.component";
import {SectoresBanxicoDeleteComponent} from "./sectores-banxico-delete/sectores-banxico-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-sectores-banxico',
  templateUrl: './sectores-banxico.component.html',
  styleUrls: ['./sectores-banxico.component.sass']
})
export class SectoresBanxicoComponent implements OnInit {
  public _datos = { _title: 'Sectores BANXICO', _modulo: 'CatalogosOficiales', _icono: 'fas fa-folder-open', _dominio: 'SectoresBanxico', _componente: 'Sectores Banxico'}
  ...
  selection = new SelectionModel<_sectoresBanxico>(true, []);
  advanceTable: _sectoresBanxico | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: SectoresBanxicoDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_sectoresBanxico>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(SectoresBanxicoFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_sectoresBanxico>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(SectoresBanxicoFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(SectoresBanxicoDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_sectoresBanxico>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new SectoresBanxicoDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _sectoresBanxico) {
...
export class SectoresBanxicoDataSource extends DataSource<_sectoresBanxico> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _sectoresBanxico[] = [];
  renderedData: _sectoresBanxico[] = [];
  ...
  connect(): Observable<_sectoresBanxico[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _sectoresBanxico) => {
          const searchStr = ( advanceTable.id + advanceTable.claveSector + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _sectoresBanxico[]): _sectoresBanxico[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveSector':
          [propertyA, propertyB] = [a.claveSector, b.claveSector];
          break;
