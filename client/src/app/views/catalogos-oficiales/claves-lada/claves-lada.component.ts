...
import {ClavesLadaFormComponent} from "./claves-lada-form/claves-lada-form.component";
import {ClavesLadaDeleteComponent} from "./claves-lada-delete/claves-lada-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-claves-lada',
  templateUrl: './claves-lada.component.html',
  styleUrls: ['./claves-lada.component.sass']
})
export class ClavesLadaComponent implements OnInit {
  public _datos = { _title: 'Claves Lada', _modulo: 'CatálogosOficiales', _icono: 'fas fa-folder-open', _dominio: 'ClavesLada', _componente: 'Claves Lada'};
  ...
  selection = new SelectionModel<_clavesLada>(true, []);
  advanceTable: _clavesLada | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: ClavesLadaDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_clavesLada>().subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(ClavesLadaFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_clavesLada>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(ClavesLadaFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(ClavesLadaDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_clavesLada>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new ClavesLadaDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _clavesLada) {
...
export class ClavesLadaDataSource extends DataSource<_clavesLada> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _clavesLada[] = [];
  renderedData: _clavesLada[] = [];
  ...
  connect(): Observable<_clavesLada[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _clavesLada) => {
          const searchStr = ( advanceTable.id + advanceTable.numeroClaveLada + advanceTable.claveLadaActual ).toString().toLowerCase();
  ...
  sortData(data: _clavesLada[]): _clavesLada[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'numeroClaveLada':
          [propertyA, propertyB] = [a.numeroClaveLada, b.numeroClaveLada];
          break;
        case 'claveLadaActual':
          [propertyA, propertyB] = [a.claveLadaActual, b.claveLadaActual];
          break;
      }
