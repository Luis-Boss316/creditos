...
import {SectoresEconomicosFormComponent} from "./sectores-economicos-form/sectores-economicos-form.component";
import {SectoresEconomicosDeleteComponent} from "./sectores-economicos-delete/sectores-economicos-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-sectores-economicos',
  templateUrl: './sectores-economicos.component.html',
  styleUrls: ['./sectores-economicos.component.sass']
})
export class SectoresEconomicosComponent implements OnInit {
  public _datos = { _title: 'Sectores Economicos', _modulo: 'CatalogosOficiales', _icono: 'fas fa-folder-open', _dominio: 'SectoresEconomicos', _componente: 'Sectores Economicos'};
  displayedColumns = [ 'select',
    'claveSectorEconomico',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_sectoresEconomicos>(true, []);
  advanceTable: _sectoresEconomicos | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: SectoresEconomicosDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_sectoresEconomicos>().subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(SectoresEconomicosFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_sectoresEconomicos>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(SectoresEconomicosFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(SectoresEconomicosDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_sectoresEconomicos>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new SectoresEconomicosDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _sectoresEconomicos) {
...
export class SectoresEconomicosDataSource extends DataSource<_sectoresEconomicos> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _sectoresEconomicos[] = [];
  renderedData: _sectoresEconomicos[] = [];
  ...
  connect(): Observable<_sectoresEconomicos[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _sectoresEconomicos) => {
  ...
  sortData(data: _sectoresEconomicos[]): _sectoresEconomicos[] {
