...
export class UsuarioComponent implements OnInit {
  public _datos = { _title: 'Usuario', _modulo: 'Usuarios', _icono: 'fas fa-folder-open', _dominio: 'Usuario'}
  ...
  constructor(private router: ActivatedRoute, private dialog: MatDialog,
              ...
              private _globalService: GlobalService, private http: HttpClient) { }
  ...
  kyc(user: _usuario) {
    let data: any;
    this.advanceTableService.create<Kyc>().subscribe(result => {
      data = result;
      const dialogRef = this.dialog.open(KycFormComponent, {
        data: { title: this._datos._title, disableClose: true, data: data, action: 'Agregar' }
      });
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        formData.append('nombre', user.id.toString());
        formData.append('tipoArchivo', result.tipoArchivo);
        if (result.file) {
          formData.append('file', result.file, result.file.name);
        }

        this.http.post(this._globalService.BASE_API_URL + 'Kyc/save', formData, { headers: {
            'Authorization': 'Bearer=' + this._globalService.getAuthToken()
          }
        }).subscribe(() => {
          this.showNotification( 'snackbar-success',  'Archivo agregado!!', 'bottom', 'center' );
        }, error => {
