...
import {CoberturasGeograficasFormComponent} from "./coberturas-geograficas-form/coberturas-geograficas-form.component";
import {CoberturasGeograficasDeleteComponent} from "./coberturas-geograficas-delete/coberturas-geograficas-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-coberturas-geograficas',
  templateUrl: './coberturas-geograficas.component.html',
  styleUrls: ['./coberturas-geograficas.component.sass']
})
export class CoberturasGeograficasComponent implements OnInit {
  public _datos = { _title: 'Coberturas Geograficas', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Coberturas Geograficas', _componente: 'Coberturas Geograficas'}
  displayedColumns = [ 'select',
    'claveCobertura',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_coberturasGeograficas>(true, []);
  advanceTable: _coberturasGeograficas | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: CoberturasGeograficasDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_coberturasGeograficas>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(CoberturasGeograficasFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_coberturasGeograficas>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(CoberturasGeograficasFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(CoberturasGeograficasDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_coberturasGeograficas>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new CoberturasGeograficasDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _coberturasGeograficas) {
...
export class CoberturasGeograficasDataSource extends DataSource<_coberturasGeograficas> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _coberturasGeograficas[] = [];
  renderedData: _coberturasGeograficas[] = [];
  ...
  connect(): Observable<_coberturasGeograficas[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _coberturasGeograficas) => {
          const searchStr = ( advanceTable.id + advanceTable.claveCobertura + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _coberturasGeograficas[]): _coberturasGeograficas[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveCobertura':
          [propertyA, propertyB] = [a.claveCobertura, b.claveCobertura];
          break;
        case 'descripcion':
