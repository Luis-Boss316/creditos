...
import {TipoSeguroFormComponent} from "./tipo-seguro-form/tipo-seguro-form.component";
import {TipoSeguroDeleteComponent} from "./tipo-seguro-delete/tipo-seguro-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipo-seguro',
  templateUrl: './tipo-seguro.component.html',
  styleUrls: ['./tipo-seguro.component.sass']
})
export class TipoSeguroComponent implements OnInit {
  public _datos = { _title: 'Tipo Seguro', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Tipo Seguro', _componente: 'Tipo Seguro'}
  displayedColumns = [ 'select',
    'tipoSeguro',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_tipoSeguro>(true, []);
  advanceTable: _tipoSeguro | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TipoSeguroDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tipoSeguro>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TipoSeguroFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tipoSeguro>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TipoSeguroFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TipoSeguroDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tipoSeguro>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TipoSeguroDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tipoSeguro) {
...
export class TipoSeguroDataSource extends DataSource<_tipoSeguro> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tipoSeguro[] = [];
  renderedData: _tipoSeguro[] = [];
  ...
  connect(): Observable<_tipoSeguro[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tipoSeguro) => {
          const searchStr = ( advanceTable.id + advanceTable.tipoSeguro + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _tipoSeguro[]): _tipoSeguro[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'tipoSeguro':
          [propertyA, propertyB] = [a.tipoSeguro, b.tipoSeguro];
          break;
        case 'descripcion':
