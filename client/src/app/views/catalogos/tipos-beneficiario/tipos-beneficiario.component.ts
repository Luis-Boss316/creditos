...
import {TiposBeneficiarioFormComponent} from "./tipos-beneficiario-form/tipos-beneficiario-form.component";
import {TiposBeneficiarioDeleteComponent} from "./tipos-beneficiario-delete/tipos-beneficiario-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipos-beneficiario',
  templateUrl: './tipos-beneficiario.component.html',
  styleUrls: ['./tipos-beneficiario.component.sass']
})
export class TiposBeneficiarioComponent implements OnInit {
  public _datos = { _title: 'Tipos Beneficiario', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Tipos Beneficiario', _componente: 'Tipos Beneficiario'}
  displayedColumns = [ 'select',
    'claveTipoBeneficiario',
    'descripcion',
    'actions' ];
  selection = new SelectionModel<_tiposBeneficiario>(true, []);
  advanceTable: _tiposBeneficiario | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TiposBeneficiarioDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tiposBeneficiario>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposBeneficiarioFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tiposBeneficiario>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposBeneficiarioFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TiposBeneficiarioDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tiposBeneficiario>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TiposBeneficiarioDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tiposBeneficiario) {
...
export class TiposBeneficiarioDataSource extends DataSource<_tiposBeneficiario> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tiposBeneficiario[] = [];
  renderedData: _tiposBeneficiario[] = [];
  ...
  connect(): Observable<_tiposBeneficiario[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tiposBeneficiario) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoBeneficiario + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _tiposBeneficiario[]): _tiposBeneficiario[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoBeneficiario':
          [propertyA, propertyB] = [a.claveTipoBeneficiario, b.claveTipoBeneficiario];
          break;
        case 'descripcion':
