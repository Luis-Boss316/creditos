...
import {TiposReferenciasFormComponent} from "./tipos-referencias-form/tipos-referencias-form.component";
import {TiposReferenciasDeleteComponent} from "./tipos-referencias-delete/tipos-referencias-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipos-referencias',
  templateUrl: './tipos-referencias.component.html',
  styleUrls: ['./tipos-referencias.component.sass']
})
export class TiposReferenciasComponent implements OnInit {
  public _datos = { _title: 'Tipos Referencias', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Tipos Referencias', _componente: 'Tipos Referencias'}
  displayedColumns = [ 'select',
    'claveTipoReferencia',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_tiposReferencia>(true, []);
  advanceTable: _tiposReferencia | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TiposReferenciasDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tiposReferencia>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposReferenciasFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tiposReferencia>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposReferenciasFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TiposReferenciasDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tiposReferencia>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TiposReferenciasDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tiposReferencia) {
...
export class TiposReferenciasDataSource extends DataSource<_tiposReferencia> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tiposReferencia[] = [];
  renderedData: _tiposReferencia[] = [];
  ...
  connect(): Observable<_tiposReferencia[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tiposReferencia) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoReferencia + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _tiposReferencia[]): _tiposReferencia[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoReferencia':
          [propertyA, propertyB] = [a.claveTipoReferencia, b.claveTipoReferencia];
          break;
        case 'descripcion':
