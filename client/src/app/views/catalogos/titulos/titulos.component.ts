...
import {TitulosDeleteComponent} from "./titulos-delete/titulos-delete.component";
...
export class TitulosComponent implements OnInit {
  public _datos = { _title: 'Titulos', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Titulos', _componente: 'Titulos'}
  ...
  selection = new SelectionModel<_titulos>(true, []);
  advanceTable: _titulos | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TitulosDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_titulos>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TitulosFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_titulos>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TitulosFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TitulosDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_titulos>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TitulosDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _titulos) {
...
export class TitulosDataSource extends DataSource<_titulos> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _titulos[] = [];
  renderedData: _titulos[] = [];
  ...
  connect(): Observable<_titulos[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _titulos) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoPropiedad + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _titulos[]): _titulos[] {
