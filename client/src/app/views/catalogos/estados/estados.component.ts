...
@Component({
  ...
  styleUrls: ['./estados.component.sass']
})
export class EstadosComponent implements OnInit {
  public _datos = { _title: 'Estados', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Estados', _componente: 'Estados'}
  ...
  selection = new SelectionModel<_estados>(true, []);
  advanceTable: _estados | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: EstadosDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_estados>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(EstadosFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_estados>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(EstadosFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(EstadosDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_estados>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new EstadosDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _estados) {
...
export class EstadosDataSource extends DataSource<_estados> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _estados[] = [];
  renderedData: _estados[] = [];
  ...
  connect(): Observable<_estados[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _estados) => {
          const searchStr = ( advanceTable.id + advanceTable.claveEstado + advanceTable.numeroPais + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _estados[]): _estados[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveEstado':
          [propertyA, propertyB] = [a.claveEstado, b.claveEstado];
          break;
        case 'numeroPais':
