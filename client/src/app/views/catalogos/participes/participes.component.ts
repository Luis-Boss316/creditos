...
import {ParticipesFormComponent} from "./participes-form/participes-form.component";
import {ParticipesDeleteComponent} from "./participes-delete/participes-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-participes',
  templateUrl: './participes.component.html',
  styleUrls: ['./participes.component.sass']
})
export class ParticipesComponent implements OnInit {
  public _datos = { _title: 'Participes', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Participes', _componente: 'Participes'}
  displayedColumns = [ 'select',
    'claveParticipe',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_participes>(true, []);
  advanceTable: _participes | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: ParticipesDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_participes>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(ParticipesFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_participes>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(ParticipesFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(ParticipesDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_participes>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new ParticipesDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _participes) {
...
export class ParticipesDataSource extends DataSource<_participes> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _participes[] = [];
  renderedData: _participes[] = [];
  ...
  connect(): Observable<_participes[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _participes) => {
          const searchStr = ( advanceTable.id + advanceTable.claveParticipe + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _participes[]): _participes[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveParticipe':
          [propertyA, propertyB] = [a.claveParticipe, b.claveParticipe];
          break;
        case 'descripcion':
