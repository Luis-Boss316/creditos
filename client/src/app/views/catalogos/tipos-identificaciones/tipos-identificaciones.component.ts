...
import {TiposIdentificacionesFormComponent} from "./tipos-identificaciones-form/tipos-identificaciones-form.component";
import {TiposIdentificacionesDeleteComponent} from "./tipos-identificaciones-delete/tipos-identificaciones-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipos-identificaciones',
  templateUrl: './tipos-identificaciones.component.html',
  styleUrls: ['./tipos-identificaciones.component.sass']
})
export class TiposIdentificacionesComponent implements OnInit {
  public _datos = { _title: 'Tipos Identificaciones', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Tipos Identificaciones', _componente: 'Tipos Identificaciones'}
  displayedColumns = [ 'select',
    'claveTipoIdentificacion',
    'descripcionCorta',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_tiposIdentificaciones>(true, []);
  advanceTable: _tiposIdentificaciones | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TiposIdentificacionesDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tiposIdentificaciones>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposIdentificacionesFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tiposIdentificaciones>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposIdentificacionesFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TiposIdentificacionesDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tiposIdentificaciones>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TiposIdentificacionesDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tiposIdentificaciones) {
...
export class TiposIdentificacionesDataSource extends DataSource<_tiposIdentificaciones> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tiposIdentificaciones[] = [];
  renderedData: _tiposIdentificaciones[] = [];
  ...
  connect(): Observable<_tiposIdentificaciones[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tiposIdentificaciones) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoIdentificacion + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _tiposIdentificaciones[]): _tiposIdentificaciones[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoIdentificacion':
          [propertyA, propertyB] = [a.claveTipoIdentificacion, b.claveTipoIdentificacion];
          break;
