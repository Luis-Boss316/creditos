...
import {_modoContacto} from "../../../shared/interfaces/Creditos.interface";
...
import {ModoContactoFormComponent} from "./modo-contacto-form/modo-contacto-form.component";
import {ModoContactoDeleteComponent} from "./modo-contacto-delete/modo-contacto-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-modo-contacto',
  templateUrl: './modo-contacto.component.html',
  styleUrls: ['./modo-contacto.component.sass']
})
export class ModoContactoComponent implements OnInit {
  public _datos = { _title: 'Modo de Contacto',
    _modulo: 'Catalogos',
    _icono: 'fas fa-folder-open',
    _dominio: 'ModoContacto',
    _componente: 'Modo-Contacto'}
  ...
  selection = new SelectionModel<_modoContacto>(true, []);
  advanceTable: _modoContacto | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: ModoContactoDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_modoContacto>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(ModoContactoFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_modoContacto>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(ModoContactoFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(ModoContactoDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_modoContacto>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new ModoContactoDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _modoContacto) {
...
export class ModoContactoDataSource extends DataSource<_modoContacto> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _modoContacto[] = [];
  renderedData: _modoContacto[] = [];
  ...
  connect(): Observable<_modoContacto[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _modoContacto) => {
          const searchStr = ( advanceTable.id + advanceTable.valor + advanceTable.descripcion + advanceTable.financieraNacional ).toLowerCase();
  ...
  sortData(data: _modoContacto[]): _modoContacto[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case 'descripcion':
          ...
          break;
        case 'financieraNacional':
          [propertyA, propertyB] = [a.financieraNacional, b.financieraNacional];
          break;
      }
