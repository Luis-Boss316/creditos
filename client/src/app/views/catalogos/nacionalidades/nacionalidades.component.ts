...
import {NacionalidadesFormComponent} from "./nacionalidades-form/nacionalidades-form.component";
import {NacionalidadesDeleteComponent} from "./nacionalidades-delete/nacionalidades-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-nacionalidades',
  templateUrl: './nacionalidades.component.html',
  styleUrls: ['./nacionalidades.component.sass']
})
export class NacionalidadesComponent implements OnInit {
  public _datos = { _title: 'Nacionalidades', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Nacionalidades', _componente: 'Nacionalidades'}
  ...
  selection = new SelectionModel<_nacionalidades>(true, []);
  advanceTable: _nacionalidades | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: NacionalidadesDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_nacionalidades>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(NacionalidadesFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_nacionalidades>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(NacionalidadesFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(NacionalidadesDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_nacionalidades>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new NacionalidadesDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _nacionalidades) {
...
export class NacionalidadesDataSource extends DataSource<_nacionalidades> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _nacionalidades[] = [];
  renderedData: _nacionalidades[] = [];
  ...
  connect(): Observable<_nacionalidades[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _nacionalidades) => {
          const searchStr = ( advanceTable.id + advanceTable.claveNacionalidad + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _nacionalidades[]): _nacionalidades[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveNacionalidad':
          [propertyA, propertyB] = [a.claveNacionalidad, b.claveNacionalidad];
          break;
