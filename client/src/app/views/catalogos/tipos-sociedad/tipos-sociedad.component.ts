...
import {TiposSociedadFormComponent} from "./tipos-sociedad-form/tipos-sociedad-form.component";
import {TiposSociedadDeleteComponent} from "./tipos-sociedad-delete/tipos-sociedad-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipos-sociedad',
  templateUrl: './tipos-sociedad.component.html',
  styleUrls: ['./tipos-sociedad.component.sass']
})
export class TiposSociedadComponent implements OnInit {
  public _datos = { _title: 'Tipos Sociedad', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Tipos Sociedad', _componente: 'Tipos Sociedad'}
  ...
  selection = new SelectionModel<_tiposSociedad>(true, []);
  advanceTable: _tiposSociedad | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TiposSociedadDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tiposSociedad>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposSociedadFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tiposSociedad>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposSociedadFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TiposSociedadDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tiposSociedad>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TiposSociedadDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tiposSociedad) {
...
export class TiposSociedadDataSource extends DataSource<_tiposSociedad> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tiposSociedad[] = [];
  renderedData: _tiposSociedad[] = [];
  ...
  connect(): Observable<_tiposSociedad[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tiposSociedad) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoSociedad + advanceTable.descripcion + advanceTable.descripcionLarga ).toLowerCase();
  ...
  sortData(data: _tiposSociedad[]): _tiposSociedad[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoSociedad':
          [propertyA, propertyB] = [a.claveTipoSociedad, b.claveTipoSociedad];
          break;
        case 'descripcion':
