...
import {MunicipiosFormComponent} from "./municipios-form/municipios-form.component";
...
export class MunicipiosComponent implements OnInit {
  public _datos = { _title: 'Municipios', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Municipios', _componente: 'Municipios'}
  ...
  selection = new SelectionModel<_municipios>(true, []);
  advanceTable: _municipios | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: MunicipiosDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_municipios>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(MunicipiosFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_municipios>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(MunicipiosFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(MunicipiosDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_municipios>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new MunicipiosDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _municipios) {
...
export class MunicipiosDataSource extends DataSource<_municipios> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _municipios[] = [];
  renderedData: _municipios[] = [];
  ...
  connect(): Observable<_municipios[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _municipios) => {
          const searchStr = ( advanceTable.id + advanceTable.numeroMunicipio + advanceTable.numeroEstado + advanceTable.numeroPais + advanceTable.descripcionEstado ).toLowerCase();
  ...
  sortData(data: _municipios[]): _municipios[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'numeroMunicipio':
          [propertyA, propertyB] = [a.numeroMunicipio, b.numeroMunicipio];
          break;
        case 'numeroEstado':
