...
import {OrigenIngresosFormComponent} from "./origen-ingresos-form/origen-ingresos-form.component";
import {OrigenIngresosDeleteComponent} from "./origen-ingresos-delete/origen-ingresos-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-origen-ingresos',
  templateUrl: './origen-ingresos.component.html',
  styleUrls: ['./origen-ingresos.component.sass']
})
export class OrigenIngresosComponent implements OnInit {
  public _datos = { _title: 'Origen Ingresos', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Origen Ingresos', _componente: 'Origen Ingresos'}
  displayedColumns = [ 'select',
    'claveOrigenIngresos',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_origenIngresos>(true, []);
  advanceTable: _origenIngresos | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: OrigenIngresosDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_origenIngresos>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(OrigenIngresosFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_origenIngresos>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(OrigenIngresosFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(OrigenIngresosDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_origenIngresos>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new OrigenIngresosDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _origenIngresos) {
...
export class OrigenIngresosDataSource extends DataSource<_origenIngresos> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _origenIngresos[] = [];
  renderedData: _origenIngresos[] = [];
  ...
  connect(): Observable<_origenIngresos[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _origenIngresos) => {
          const searchStr = ( advanceTable.id + advanceTable.claveOrigenIngresos + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _origenIngresos[]): _origenIngresos[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveOrigenIngresos':
          [propertyA, propertyB] = [a.claveOrigenIngresos, b.claveOrigenIngresos];
          break;
        case 'descripcion':
