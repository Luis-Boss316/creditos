...
import {ModoContactoDataSource} from "../modo-contacto/modo-contacto.component";
...
import {SucursalesDeleteComponent} from "./sucursales-delete/sucursales-delete.component";
...
export class SucursalesComponent implements OnInit {
  public _datos = { _title: 'Sucursales',
    _modulo: 'Catalogos',
    _icono: 'fas fa-folder-open',
    _dominio: 'Sucursales',
    _componente: 'Sucursales'}
  ...
  selection = new SelectionModel<_sucursales>(true, []);
  advanceTable: _sucursales | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: SucursalesDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_sucursales>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(SucursalesFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_sucursales>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(SucursalesFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(SucursalesDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_sucursales>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new SucursalesDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _sucursales) {
...
export class SucursalesDataSource extends DataSource<_sucursales> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _sucursales[] = [];
  renderedData: _sucursales[] = [];
  ...
  connect(): Observable<_sucursales[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _sucursales) => {
          const searchStr = ( advanceTable.id + advanceTable.sucursal + advanceTable.responsable + advanceTable.domicilio + advanceTable.telefono ).toLowerCase();
  ...
  sortData(data: _sucursales[]): _sucursales[] {
