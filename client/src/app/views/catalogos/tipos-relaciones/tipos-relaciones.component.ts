...
import {TiposRelacionesFormComponent} from "./tipos-relaciones-form/tipos-relaciones-form.component";
import {TiposRelacionesDeleteComponent} from "./tipos-relaciones-delete/tipos-relaciones-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipos-relaciones',
  templateUrl: './tipos-relaciones.component.html',
  styleUrls: ['./tipos-relaciones.component.sass']
})
export class TiposRelacionesComponent implements OnInit {
  public _datos = { _title: 'Tipos Relaciones', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Tipos Relaciones', _componente: 'Tipos Relaciones'}
  displayedColumns = [ 'select',
    'claveTipoRelacion',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_tiposRelaciones>(true, []);
  advanceTable: _tiposRelaciones | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TiposRelacionesDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tiposRelaciones>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposRelacionesFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tiposRelaciones>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposRelacionesFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TiposRelacionesDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tiposRelaciones>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TiposRelacionesDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tiposRelaciones) {
...
export class TiposRelacionesDataSource extends DataSource<_tiposRelaciones> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tiposRelaciones[] = [];
  renderedData: _tiposRelaciones[] = [];
  ...
  connect(): Observable<_tiposRelaciones[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tiposRelaciones) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoRelacion + advanceTable.descripcion).toLowerCase();
  ...
  sortData(data: _tiposRelaciones[]): _tiposRelaciones[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoRelacion':
          [propertyA, propertyB] = [a.claveTipoRelacion, b.claveTipoRelacion];
          break;
        case 'descripcion':
