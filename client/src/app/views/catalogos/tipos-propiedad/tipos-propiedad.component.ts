...
import {TiposPropiedadFormComponent} from "./tipos-propiedad-form/tipos-propiedad-form.component";
import {TiposPropiedadDeleteComponent} from "./tipos-propiedad-delete/tipos-propiedad-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipos-propiedad',
  templateUrl: './tipos-propiedad.component.html',
  styleUrls: ['./tipos-propiedad.component.sass']
})
export class TiposPropiedadComponent implements OnInit {
  public _datos = { _title: 'Tipos Propiedad', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Tipos Propiedad', _componente: 'Tipos Propiedad'}
  displayedColumns = [ 'select',
    'claveTipoPropiedad',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_tiposPropiedad>(true, []);
  advanceTable: _tiposPropiedad | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TiposPropiedadDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tiposPropiedad>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposPropiedadFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tiposPropiedad>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposPropiedadFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TiposPropiedadDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tiposPropiedad>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TiposPropiedadDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tiposPropiedad) {
...
export class TiposPropiedadDataSource extends DataSource<_tiposPropiedad> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tiposPropiedad[] = [];
  renderedData: _tiposPropiedad[] = [];
  ...
  connect(): Observable<_tiposPropiedad[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tiposPropiedad) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoPropiedad + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _tiposPropiedad[]): _tiposPropiedad[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoPropiedad':
          [propertyA, propertyB] = [a.claveTipoPropiedad, b.claveTipoPropiedad];
          break;
        case 'descripcion':
