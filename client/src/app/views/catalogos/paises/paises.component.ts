...
export class PaisesComponent implements OnInit {
  public _datos = { _title: 'Paises', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Paises', _componente: 'Paises'}
  ...
  selection = new SelectionModel<_paises>(true, []);
  advanceTable: _paises | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: PaisesDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_paises>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(PaisesFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_paises>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(PaisesFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(PaisesDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_paises>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new PaisesDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _paises) {
...
export class PaisesDataSource extends DataSource<_paises> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _paises[] = [];
  renderedData: _paises[] = [];
  ...
  connect(): Observable<_paises[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _paises) => {
          const searchStr = ( advanceTable.id + advanceTable.clavePais + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'clavePais':
          [propertyA, propertyB] = [a.clavePais, b.clavePais];
          break;
