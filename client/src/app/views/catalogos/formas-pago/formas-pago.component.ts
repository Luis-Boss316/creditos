...
import {FormasPagoFormComponent} from "./formas-pago-form/formas-pago-form.component";
import {CiudadesDeleteComponent} from "../ciudades/ciudades-delete/ciudades-delete.component";
import {FormasPagoDeleteComponent} from "./formas-pago-delete/formas-pago-delete.component";
...
export class FormasPagoComponent implements OnInit {
  public _datos = { _title: 'Formas de Pago',
    _modulo: 'Catalogos',
    _icono: 'fas fa-folder-open',
    _dominio: 'FormasPago',
    _componente: 'Formas de Pago'}
  ...
  selection = new SelectionModel<_formasPago>(true, []);
  advanceTable: _formasPago | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: FormasPagoDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_formasPago>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(FormasPagoFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_formasPago>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(FormasPagoFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(FormasPagoDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_formasPago>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new FormasPagoDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _formasPago) {
...
export class FormasPagoDataSource extends DataSource<_formasPago> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _formasPago[] = [];
  renderedData: _formasPago[] = [];
  ...
  connect(): Observable<_formasPago[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _formasPago) => {
          const searchStr = ( advanceTable.id + advanceTable.valor + advanceTable.descripcion + advanceTable.operacion + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _formasPago[]): _formasPago[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case 'descripcion':
          ...
          break;
        case 'operacion':
          [propertyA, propertyB] = [a.operacion, b.operacion];
          break;
      }
