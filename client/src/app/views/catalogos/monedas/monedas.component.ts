...
import {_monedas} from "../../../shared/interfaces/Creditos.interface";
...
import {MonedasFormComponent} from "./monedas-form/monedas-form.component";
import {MonedasDeleteComponent} from "./monedas-delete/monedas-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-monedas',
  templateUrl: './monedas.component.html',
  styleUrls: ['./monedas.component.sass']
})
export class MonedasComponent implements OnInit {
  public _datos = { _title: 'Monedas', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Monedas', _componente: 'Monedas'}
  ...
  selection = new SelectionModel<_monedas>(true, []);
  advanceTable: _monedas | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: MonedasDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_monedas>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(MonedasFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_monedas>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(MonedasFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(MonedasDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_monedas>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new MonedasDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _monedas) {
...
export class MonedasDataSource extends DataSource<_monedas> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _monedas[] = [];
  renderedData: _monedas[] = [];
  ...
  connect(): Observable<_monedas[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _monedas) => {
          const searchStr = ( advanceTable.id+ advanceTable.claveMoneda+ advanceTable.descripcionCorta+ advanceTable.descripcion+ advanceTable.numeroPais ).toLowerCase();
  ...
  sortData(data: _monedas[]): _monedas[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveMoneda':
          [propertyA, propertyB] = [a.claveMoneda, b.claveMoneda];
          break;
