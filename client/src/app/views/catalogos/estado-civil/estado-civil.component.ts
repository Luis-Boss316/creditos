...
import {EstadoCivilDeleteComponent} from "./estado-civil-delete/estado-civil-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-estado-civil',
  templateUrl: './estado-civil.component.html',
  styleUrls: ['./estado-civil.component.sass']
})
export class EstadoCivilComponent implements OnInit {
  public _datos = { _title: 'Estado Civil', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Estado Civil', _componente: 'Estado Civil'}
  ...
  selection = new SelectionModel<_estadoCivil>(true, []);
  advanceTable: _estadoCivil | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: EstadoCivilDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_estadoCivil>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(EstadoCivilFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_estadoCivil>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(EstadoCivilFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(EstadoCivilDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_estadoCivil>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new EstadoCivilDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _estadoCivil) {
...
export class EstadoCivilDataSource extends DataSource<_estadoCivil> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _estadoCivil[] = [];
  renderedData: _estadoCivil[] = [];
  ...
  connect(): Observable<_estadoCivil[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _estadoCivil) => {
          const searchStr = ( advanceTable.id + advanceTable.claveEstadoCivil + advanceTable.descripcionCorta + advanceTable.descripcion + advanceTable.casado ).toLowerCase();
  ...
  sortData(data: _estadoCivil[]): _estadoCivil[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveEstadoCivil':
          [propertyA, propertyB] = [a.claveEstadoCivil, b.claveEstadoCivil];
          break;
