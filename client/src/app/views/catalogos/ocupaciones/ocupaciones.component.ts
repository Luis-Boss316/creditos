...
import {_nacionalidades, _ocupaciones} from "../../../shared/interfaces/Creditos.interface";
...
import {OcupacionesFormComponent} from "./ocupaciones-form/ocupaciones-form.component";
import {OcupacionesDeleteComponent} from "./ocupaciones-delete/ocupaciones-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-ocupaciones',
  templateUrl: './ocupaciones.component.html',
  styleUrls: ['./ocupaciones.component.sass']
})
export class OcupacionesComponent implements OnInit {
  public _datos = { _title: 'Ocupaciones', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Ocupaciones', _componente: 'Ocupaciones'}
  displayedColumns = [ 'select',
    'claveOcupacion',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_ocupaciones>(true, []);
  advanceTable: _ocupaciones | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: OcupacionesDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_ocupaciones>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(OcupacionesFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_ocupaciones>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(OcupacionesFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(OcupacionesDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_ocupaciones>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new OcupacionesDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _ocupaciones) {
...
export class OcupacionesDataSource extends DataSource<_ocupaciones> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _ocupaciones[] = [];
  renderedData: _ocupaciones[] = [];
  ...
  connect(): Observable<_ocupaciones[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _ocupaciones) => {
          const searchStr = ( advanceTable.id + advanceTable.claveOcupacion + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _ocupaciones[]): _ocupaciones[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveOcupacion':
          [propertyA, propertyB] = [a.claveOcupacion, b.claveOcupacion];
          break;
        case 'descripcion':
