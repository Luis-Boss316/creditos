...
import {TipoPersonaFormComponent} from "./tipo-persona-form/tipo-persona-form.component";
import {TipoPersonaDeleteComponent} from "./tipo-persona-delete/tipo-persona-delete.component";
...
export class TipoPersonaComponent implements OnInit {
  public _datos = { _title: 'Tipo Persona', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Tipo Persona', _componente: 'Tipo Persona'}
  ...
  selection = new SelectionModel<_tipoPersona>(true, []);
  advanceTable: _tipoPersona | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TipoPersonaDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tipoPersona>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TipoPersonaFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tipoPersona>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TipoPersonaFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TipoPersonaDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tipoPersona>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TipoPersonaDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tipoPersona) {
...
export class TipoPersonaDataSource extends DataSource<_tipoPersona> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tipoPersona[] = [];
  renderedData: _tipoPersona[] = [];
  ...
  connect(): Observable<_tipoPersona[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tipoPersona) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoPersona + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _tipoPersona[]): _tipoPersona[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoPersona':
          [propertyA, propertyB] = [a.claveTipoPersona, b.claveTipoPersona];
          break;
