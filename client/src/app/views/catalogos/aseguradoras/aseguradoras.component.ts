...
import {AseguradorasDeleteComponent} from "./aseguradoras-delete/aseguradoras-delete.component";
...
export class AseguradorasComponent implements OnInit {
  public _datos = { _title: 'Aseguradoras', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Aseguradoras', _componente: 'Aseguradoras'}

  displayedColumns = [ 'select',
    'claveAseguradoraSeguro',
    'descripcionCorta',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_aseguradoras>(true, []);
  advanceTable: _aseguradoras | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: AseguradorasDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_aseguradoras>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(AseguradorasFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_aseguradoras>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(AseguradorasFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(AseguradorasDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_aseguradoras>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new AseguradorasDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _aseguradoras) {
...
export class AseguradorasDataSource extends DataSource<_aseguradoras> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _aseguradoras[] = [];
  renderedData: _aseguradoras[] = [];
  ...
  connect(): Observable<_aseguradoras[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _aseguradoras) => {
          const searchStr = ( advanceTable.id + advanceTable.claveAseguradoraSeguro + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _aseguradoras[]): _aseguradoras[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveAseguradoraSeguro':
          [propertyA, propertyB] = [a.claveAseguradoraSeguro, b.claveAseguradoraSeguro];
          break;
