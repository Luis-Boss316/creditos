...
import {GiroEmpresarialFormComponent} from "./giro-empresarial-form/giro-empresarial-form.component";
import {GiroEmpresarialDeleteComponent} from "./giro-empresarial-delete/giro-empresarial-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-giro-empresarial',
  templateUrl: './giro-empresarial.component.html',
  styleUrls: ['./giro-empresarial.component.sass']
})
export class GiroEmpresarialComponent implements OnInit {
  public _datos = { _title: 'Giro Empresarial', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Giro Empresarial', _componente: 'Giro Empresarial'}
  displayedColumns = [ 'select',
    'claveGiroEmpresarial',
    'descripcionCorta',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_giroEmpresarial>(true, []);
  advanceTable: _giroEmpresarial | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: GiroEmpresarialDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_giroEmpresarial>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(GiroEmpresarialFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_giroEmpresarial>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(GiroEmpresarialFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(GiroEmpresarialDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_giroEmpresarial>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new GiroEmpresarialDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _giroEmpresarial) {
...
export class GiroEmpresarialDataSource extends DataSource<_giroEmpresarial> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _giroEmpresarial[] = [];
  renderedData: _giroEmpresarial[] = [];
  ...
  connect(): Observable<_giroEmpresarial[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _giroEmpresarial) => {
          const searchStr = ( advanceTable.id + advanceTable.claveGiroEmpresarial + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _giroEmpresarial[]): _giroEmpresarial[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveGiroEmpresarial':
          [propertyA, propertyB] = [a.claveGiroEmpresarial, b.claveGiroEmpresarial];
          break;
