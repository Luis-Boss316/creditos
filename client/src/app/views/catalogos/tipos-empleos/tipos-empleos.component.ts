...
import {TiposEmpleosFormComponent} from "./tipos-empleos-form/tipos-empleos-form.component";
import {TiposEmpleosDeleteComponent} from "./tipos-empleos-delete/tipos-empleos-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-tipos-empleos',
  templateUrl: './tipos-empleos.component.html',
  styleUrls: ['./tipos-empleos.component.sass']
})
export class TiposEmpleosComponent implements OnInit {
  public _datos = { _title: 'Tipos Empleos', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Tipos Empleos', _componente: 'Tipos Empleos'}
  displayedColumns = [ 'select',
    'claveTipoEmpleo',
    'descripcion',
    'actions' ];

  selection = new SelectionModel<_tiposEmpleos>(true, []);
  advanceTable: _tiposEmpleos | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: TiposEmpleosDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_tiposEmpleos>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposEmpleosFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_tiposEmpleos>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(TiposEmpleosFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(TiposEmpleosDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_tiposEmpleos>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new TiposEmpleosDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _tiposEmpleos) {
...
export class TiposEmpleosDataSource extends DataSource<_tiposEmpleos> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _tiposEmpleos[] = [];
  renderedData: _tiposEmpleos[] = [];
  ...
  connect(): Observable<_tiposEmpleos[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _tiposEmpleos) => {
          const searchStr = ( advanceTable.id + advanceTable.claveTipoEmpleo + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _tiposEmpleos[]): _tiposEmpleos[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveTipoEmpleo':
          [propertyA, propertyB] = [a.claveTipoEmpleo, b.claveTipoEmpleo];
          break;
        case 'descripcion':
