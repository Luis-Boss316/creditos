...
import {_periodicidades, _profesiones} from "../../../shared/interfaces/Creditos.interface";
...
import {ProfesionesDeleteComponent} from "./profesiones-delete/profesiones-delete.component";
import {map} from "rxjs/operators";

@Component({
  selector: 'app-profesiones',
  templateUrl: './profesiones.component.html',
  styleUrls: ['./profesiones.component.sass']
})
export class ProfesionesComponent implements OnInit {
  public _datos = { _title: 'Profesiones', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'Profesiones', _componente: 'Profesiones'}
  ...
  selection = new SelectionModel<_profesiones>(true, []);
  advanceTable: _profesiones | null;

  id: number;
  public getRowsSub: Subscription;
  db: AdvanceRestService;
  dataSource: ProfesionesDataSource | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<_profesiones>().subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(ProfesionesFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<_profesiones>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable)
      const dialogRef = this.dialog.open(ProfesionesFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(ProfesionesDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.selection = new SelectionModel<_profesiones>(true, []);
    });
    this.loadData();
  ...
  public loadData() {
    ...
    this.dataSource = new ProfesionesDataSource( this.db, this.paginator, this.sort, this._datos._dominio );
  ...
  onContextMenu(event: MouseEvent, item: _profesiones) {
...
export class ProfesionesDataSource extends DataSource<_profesiones> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: _profesiones[] = [];
  renderedData: _profesiones[] = [];
  ...
  connect(): Observable<_profesiones[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: _profesiones) => {
          const searchStr = ( advanceTable.id + advanceTable.claveProfesion + advanceTable.descripcionCorta + advanceTable.descripcion ).toLowerCase();
  ...
  sortData(data: _profesiones[]): _profesiones[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'claveProfesion':
          [propertyA, propertyB] = [a.claveProfesion, b.claveProfesion];
          break;
